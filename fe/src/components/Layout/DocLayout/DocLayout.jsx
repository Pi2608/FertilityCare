...
const DocLayout = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    const lastPath = localStorage.getItem("doctorLastVisited");

    // Only redirect if on the root dashboard
    if (location.pathname === "/doctor-dashboard" && lastPath) {
        ...
        {shouldShowSidebar && (
          <aside className="sidebar">
            <div className="sidebar-header">
              ...
              <p className="sidebar-subtitle">Cổng thông tin Bác sĩ</p>
            </div>
            <nav className="sidebar-nav">
              {list.map((item, index) => (
                <Link
                  ...
                  to={item.path}
                  onClick={() =>
                    localStorage.setItem("doctorLastVisited", item.path)
                  }
