...
const PatientProfileLayout = () => {
  ...
  const patientData = currentCycle
    ? {
        name: appointmentDetail.customerName || "Đang tải...",
        id: `PT-${currentCycle.customerId}`,
        treatment: currentCycle.serviceName, // Lấy từ currentCycle.serviceName
        status: currentCycle.status === "done" ? "Hoàn thành" : "Đang điều trị",
        age: appointmentDetail.customerAge || 0,
        birthDate: "",
        gender: "",
        phone: "",
        email: "",
        ...
        startDate: currentCycle.startDate
          ? new Date(currentCycle.startDate).toLocaleDateString("vi-VN")
          : "",
  ...
  const getCurrentStepPeriod = (stepOrder) => {
    ...
    if (step?.startDate && step?.eventDate) {
      return `${new Date(step.startDate).toLocaleDateString(
        "vi-VN"
      )} - ${new Date(step.eventDate).toLocaleDateString("vi-VN")}`;
    } else if (step) {
      return `${new Date(step.startDate).toLocaleDateString(
        "vi-VN"
      )} - Hiện tại`;
  ...
  const filterFirstOngoingStep = (steps) => {
    const finishedPhases = steps.filter(
      (p) => p.statusCycleStep === "finished"
    );
    const firstOngoingPhase = steps.find(
      (p) => p.statusCycleStep === "ongoing"
    );
    const firstStoppedPhase = steps.find(
      (p) => p.statusCycleStep === "stopped"
    );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      await Promise.all([getCustomerTestResults(customerId)]);
        ...
        <div className="patient-profile-treatment-cards">
          <div className="patient-profile-treatment-card patient-profile-current">
            <div className="patient-profile-card-icon">
              <span className="patient-profile-icon">
            ...
            <div className="patient-profile-card-content">
              ...
              <p>
                {allCycleStep.length > currentStep(allCycleStep)
                  ? mappingStepsName(currentStep(allCycleStep))
          ...
          {allCycleStep.length > currentStep(allCycleStep) && (
            <div className="patient-profile-treatment-card patient-profile-next">
              <div className="patient-profile-card-icon">
                <span className="patient-profile-icon">
                  <CalendarDays size={24} color="#2483ffff" />
                </span>
              </div>
              <div className="patient-profile-card-content">
        ...
        <div className="patient-profile-treatment-timeline">
          ...
          <div className="patient-profile-timeline">
            {filterFirstOngoingStep(allCycleStep)?.map((phase, idx) => (
              <div
                key={`key-${phase.stepId}-${idx}`}
                className={`patient-profile-timeline-item patient-profile-${
              ...
              >
                ...
                <div className="patient-profile-timeline-content">
                  <div className="patient-profile-timeline-header">
                    <h4>
                      Giai đoạn {phase.stepOrder}:{" "}
                      {mappingStepsName(phase.stepOrder)}
                    </h4>
                    <span className="patient-profile-timeline-date">
                      {getCurrentStepPeriod(phase.stepOrder)}
                    </span>
                  </div>
                  <div className="patient-profile-timeline-details">
                    ...
                    {phase?.failedReason &&
                      phase.statusCycleStep === "stopped" && (
                        <div
                          className="patient-profile-timeline-section"
                    ...
                    {phase?.failedReason &&
                      (phase.statusCycleStep === "ongoing" ||
                        ...
                        >
                          {phase?.failedReason &&
                            phase.statusCycleStep === "ongoing" && (
                              <AlertTriangle size={18} color="#ffcd37" />
                            )}
                          <p
                            style={
                              phase.statusCycleStep === "ongoing"
                    ...
                    {groupByCycleStepId(testResult, phase.stepId).length > 0 ? (
                      <div className="patient-profile-timeline-section">
                        <ul>
                          {groupByCycleStepId(testResult, phase.stepId).map(
                            (result, index) => (
                              <li key={index}>
                      ...
                      <div className="patient-profile-timeline-section">
                        <p>
                          {phase.statusCycleStep === "ongoing"
                            ? "Chưa"
                            : "Không"}{" "}
                          có kết quả xét nghiệm cho giai đoạn này.
                        </p>
                      </div>
                    )}
                      ...
                      <div className="patient-profile-timeline-section">
                        <p>
                          {phase.statusCycleStep === "ongoing"
                            ? "Chưa"
                            : "Không"}{" "}
                          có thuốc sử dụng cho giai đoạn này.
                        </p>
                      </div>
                    )}
                  </div>
            ...
            currentCycle?.status !== "stopped" && (
              ...
              >
                <div
                  className="patient-profile-timeline-marker"
                  style={{ background: "#a9b5c6", color: "white" }}
                >
                  <CalendarDays size={20} />
                </div>
                <div className="patient-profile-timeline-content">
                  <div className="patient-profile-timeline-header">
                    <h4>
                      Giai đoạn{" "}
                      {filterFirstOngoingStep(allCycleStep).length + 1}:{" "}
                      {mappingStepsName(
                        filterFirstOngoingStep(allCycleStep).length + 1
                      )}
                    </h4>
                  ...
                  <div className="patient-profile-timeline-section">
                    <div className="patient-profile-timeline-note">
                      <p style={{ marginBottom: "1rem" }}>
                        Giai đoạn này sẽ bắt đầu sau khi hoàn thành giai đoạn
                        hiện tại.
                      </p>
                    </div>
                  </div>
            ...
            (phase) =>
              phase?.note && (
                <>
                  <div
                    key={`${phase.id}`}
                    className="patient-profile-note-item"
                  >
              ...
              <div className="patient-profile-results-list">
                ...
                >
                  ...
                  <div className="patient-profile-result-details">
                    ...
                    <p>
                      Kết quả: {phase.value} {phase.unit}
                    </p>
                    <p>
        ...
        <div className="patient-profile-medication-cards">
          {allCycleStep
            ...
            .sort((a, b) => {
              ...
              return (
                priority[b.statusInfo.class] - priority[a.statusInfo.class]
              );
            })
            .map(({ key, statusInfo, step, med }) => (
              ...
              >
                <div className="patient-profile-med-header">
                  ...
                  <span
                    className={`patient-profile-status-badge patient-profile-${statusInfo.class}`}
                  >
                    {statusInfo.text}
                  </span>
                </div>
                <div className="patient-profile-med-details">
                  ...
                  <p>
                    ...
                    {mappingStepsName(step.stepOrder)}
                  </p>
    ...
    useEffect(() => {
      if (currentCycle) {
        setFormData((prev) => ({
          ...
          cycleStepId:
            allCycleStep?.[currentStep(allCycleStep) - 1]?.stepId || "",
        }));
      }
    }, [currentCycle]);
  ...
  const CreateMedicationScheduleModal = memo(({ isOpen, onClose }) => {
    const FIXED_TIME_SLOTS = ["08:00", "12:00", "20:00"];

    const minDate = new Date(Date.now()).toISOString().split("T")[0];
    const maxDate = new Date(
      currentCycle?.cycleStep[currentStep(allCycleStep) - 1]?.eventDate
    );
    ...
    const handleCreateMedicationSchedule = useCallback(
      async (e) => {
        e.preventDefault();
        setIsLoading(true);
        try {
  ...
  const UpdateCycleStepNoteModal = memo(({ isOpen, onClose, form }) => {
    ...
    const handleUpdateCycleStepNote = useCallback(
      async (e) => {
        e.preventDefault();
        setIsLoading(true);
        try {
  ...
  const NewCycleModal = memo(({ isOpen, onClose }) => {
    ...
    const todayStr =
      ...
      "-" +
      String(now.getDate()).padStart(2, "0");
    const currentCycl = cycleSteps?.find(
      (phase) => phase.stepOrder === currentStep(allCycleStep)
    );
    ...
    useEffect(() => {
      if (currentCycle) {
        setFormData((prev) => ({
          ...
          serviceId: currentCycle.serviceId,
          cycleStepId:
            allCycleStep?.[currentStep(allCycleStep) - 1]?.stepId + 1 || "",
        }));
      }
    }, [currentCycle]);
    ...
    const confirmSubmit = async () => {
      ...
      try {
        ...
        await updateCycleStepStatus(
          currentCycle.cycleId,
          allCycleStep?.[currentStep(allCycleStep) - 1].stepOrder,
          callbackParams
        );
        showSuccess("Đặt lịch tái khám thành công");
        onClose();
        ...
        {showConfirm && (
          <div className="patient-profile-confirm-overlay">
            <div className="patient-profile-confirm-modal">
              ...
              <div>
                <p>
                  Bạn có chắc chắn đi tới bước tiếp theo và kết thúc bước hiện
                  tại hay không?
                </p>
  ...
  const RestartCycleModal = memo(({ isOpen, onClose }) => {
    ...
    const REASONS = ["Chưa đạt điều kiện", "Gặp vấn đề phát sinh"];
    ...
    const isReasonReExam =
      ["Chưa đạt điều kiện", "Gặp vấn đề phát sinh"].includes(
        formData.reason
      ) && formData.changeDate;
